...
{ 
	...
	// 计算距离
	protected function calcul_distance($lon1, $lat1, $lon2, $lat2) {
...
/*
        ...
        // 勾股定理求斜边长
        $distance = abs(sqrt($dx * $dx + $dy * $dy));
        return $distance;*/
		$lat = (floatval($lat1) - floatval($lat2)) * (floatval($lat1) - floatval($lat2));
		$lng = (floatval($lon1) - floatval($lon2)) * (floatval($lon1) - floatval($lon2));
		return abs(sqrt($lat + $lng));
	}

	function ck() {
		echo var_dump($_COOKIE);
		echo 'lng='.ICookie::get('lng').'|| lat='.ICookie::get('lat').'|| name'.ICookie::get('mapname').'|| address='.ICookie::get('myaddress');
	}

	function index() {
		if (isset($_COOKIE['geo_lat']) && isset($_COOKIE['geo_lng'])) {
			$sql = "
			SELECT distinct a.* FROM ".Mysite::$app->config['tablepre']."area a
			left join ".Mysite::$app->config['tablepre']."areashop asp on a.id = asp.areaid
			left join ".Mysite::$app->config['tablepre']."shop sp on asp.shopid = sp.id
			where `show` = 1 and is_open = 1 and is_pass = 1 and is_recom = 1 and parent_id != 0";

	 	    $query = $this->mysql->query($sql);
	 	    $min_distance = 10000;
	 	    $once = true;
	 	    $info_log = "";
	 	    while($rc = $this->mysql->assoc($query)) {
	 	    	setCookie('key', $rc['id']);
 	    		setCookie('value', $rc['name']);
	 	     	$tmp = $this->calcul_distance($_COOKIE['geo_lng'], $_COOKIE['geo_lat'], $rc['lng'], $rc['lat']);
	 	    ...
	 	    //setCookie('log_info', $info_log);
	 	    //error_log($info_log, 3);

			ICookie::set('lng',$data['lng'],2592000);  
        	ICookie::set('lat',$data['lat'],2592000);  
            ICookie::set('mapname',$data['name'],2592000);  
            ICookie::set('myaddress',$data['address'],2592000);    
            setCookie('tmp', $tmp);
            setCookie('cnt', count($list));
	 ...
	 function shoplistdata(){
      ...
      //计算距离远近的方法
	   ...
	   if(!empty($psset)){
	    ...
	    if($locationtype == 1){
	         ...
	         if($checkareaid > 0){
	           ...
	           $bili = intval(Mysite::$app->config['servery']/1000);
		          $bili = $bili*0.009;
	         $where = empty($where)?'and   SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < '.$bili.' ': $where.' and SQRT ((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*'.$bili.')';
           //构造排序条件
           //echo $where;
	    ...
	    //error_log("select * from ".Mysite::$app->config['tablepre']."shopfast as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id  where  b.is_pass = 1 ".$where."    order by ".$orderarray[$order]." limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."", 3, 'map.log');
	...
	function buycart(){ 
     ...
     if($shopinfo['is_open'] == 0  || $shopinfo['is_pass'] == 0){
	   ...
	   $data['yanchitime'] = date('Y-m-d H:i:s',$yanchitime); 
