...
{  
	 	    ...
	 	    //搜索信息
	 	     ...
	 	     if($locationtype == 1){
	 	        ...
	 	        $lat = empty($lat)?0:$lat;  
		          ...
		          $bili = $bili*0.009;
		          $where .= empty($where) ? ' where id > 0 and endtime > '.time().' and  SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*'.$bili.') ':' and id > 0 and endtime > '.time().'  and SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*'.$bili.') ';//0.009/6 = 0.0015
		     }else{
	 	    	//文字定位
	 	    	  $nowadID = ICookie::get('myaddress'); 
	         $myaddressname= ICookie::get('mapname');  
	...
	function  ajaxlngtlat(){
		 ...
		 $lat= IFilter::act(IReq::get('lat'));
		  $bili = intval(Mysite::$app->config['servery']/1000);
		 	  $bili = $bili*0.009;
		 //$shuliang =  $this->mysql->counts("select b.id from ".Mysite::$app->config['tablepre']."shopfast as a left join ".Mysite::$app->config['tablepre']."shop as b  on b.id=a.shopid where b.is_open=1  and  endtime > ".time()." and SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradius`*0.0015) order by b.id asc limit 0,1000"); 
		 $shuliang =  $this->mysql->counts("select b.id from ".Mysite::$app->config['tablepre']."shopfast as a left join ".Mysite::$app->config['tablepre']."shop as b  on b.id=a.shopid where b.is_open=1  and  endtime > ".time()." and SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradius`*".$bili.") order by b.id asc limit 0,1000");
		 error_log("select b.id from ".Mysite::$app->config['tablepre']."shopfast as a left join ".Mysite::$app->config['tablepre']."shop as b  on b.id=a.shopid where b.is_open=1  and  endtime > ".time()." and SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradius`*".$bili.") order by b.id asc limit 0,1000",3,'map.log') ;
		  ...
		  $shuliang2 =  $this->mysql->counts("select b.id from ".Mysite::$app->config['tablepre']."shopmarket as a left join ".Mysite::$app->config['tablepre']."shop as b  on b.id=a.shopid where b.is_open=1  and  endtime > ".time()." and SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradius`*".$bili.") order by b.id asc limit 0,1000"); 
	...
	function showcatax(){
     ...
     $data['locationtype'] = $psinfo['locationtype'];
	  ...
	  $data['nowdaymy'] = date('Y-m-d',time());
	  $data['nowdaytime'] = date('Y-m-d H:i:s',time());
	  $btime = time()+$data['shopinfo']['maketime']*60;
	  $data['yanchitime'] =  date('Y-m-d H:i:s',$btime);
	   Mysite::$app->setdata($data); 
	}
	function showcart(){
		 //检测是否  在配送范围
