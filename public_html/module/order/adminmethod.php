...
//statustype  4   退款处理
    ...
    $starttime = empty($starttime)? date('Y-m-d',time()):$starttime;
    $endtime = empty($endtime)? date('Y-m-d',time()):$endtime;
	  $where = '  where ord.posttime > '.strtotime($starttime).' and ord.posttime < '.strtotime($endtime.' 23:59:59');
		 //order: id  dno 订单编号 shopuid 店铺UID shopid 店铺ID shopname 店铺名称 shopphone 店铺电话 shopaddress 店铺地址 buyeruid 购买用户ID，0未注册用户 buyername
		$where .= ' and ord.status = 0 ';
	...
	function outorderlimit(){
	 ...
	 // array('下单用户','店铺名','地址1','地址2','订单详情','商品数量','单价','联系电话','配送时间','详细地址','备注');


		 $outexcel = new phptoexcel();
		 $titledata = array('订单编号','下单用户','店铺名','地址1','地址2','商品名称','商品数量','单价','联系电话','配送时间','详细地址','备注');
