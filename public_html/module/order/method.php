   ...
   function setstatus(){
	...
	function shoporderlist(){
	  ...
	  $shopid = ICookie::get('adminshopid');
		...
		$starttime = empty($starttime)? date('Y-m-d',time()):$starttime;
		$endtime = empty($endtime)? date('Y-m-d',time()):$endtime;
		...
		$where = '';
	  $where = '  and posttime > '.strtotime($starttime.' 00:00:00').' and posttime < '.strtotime($endtime.' 23:59:59');
 ...
 function ajaxcheckshoporder(){
 	  ...
 	  $shopid = ICookie::get('adminshopid');
		...
		$endtime = empty($endtime)? date('Y-m-d',time()):$endtime;
	     ...
	     $where = '  and posttime > '.strtotime($starttime.' 00:00:00').' and posttime < '.strtotime($endtime.' 23:59:59');
	   $tempshu =  $this->mysql->select_one("select count(id) as shuliang  from ".Mysite::$app->config['tablepre']."order where shopid='".$shopid."' and  status > 0  and  status <  4  ".$where." limit 0,1000");
