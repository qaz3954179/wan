...
function carttoggle(){
	$("#gouwuchetoggle").toggle();
	 var o =document.getElementById("gouwuchetoggle").style.display;
		if(o=="none"){
			$(".fshetotal").show();
		}else{
			$(".fshetotal").hide();
...
function cartimg(obj){   
       ...
       $(obj).find('.listnumber').eq(0).css({width:'20px',height:'20px'});
...
function cartstate(){
	var len = $("#newcart .new_cart_items .new_cart_show").length;
	if (len > 6) {
		$("#newcart").addClass('new_cart_overflow');
	} else {
		$("#newcart").removeClass('new_cart_overflow');
	}
}
...
function freshcart(){ 
	...
	/* 	 $('.AddOrder').find('.listnumber').eq(0).css({width:'30px',height:'30px'});
              var shuliang = Number($('.AddOrder').find('.listnumber').eq(0).find('span').eq(0).text())+Number(val.count);
              $('.AddOrder').find('.listnumber').eq(0).find('span').eq(0).text(shuliang); */
