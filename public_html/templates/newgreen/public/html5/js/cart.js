	     ...
	     if($('#addShopping').html() != undefined){
          ...
          if(datas.content.canps != 1){
  		  ...
  		  if($('#addShopping').html() != undefined){
  		  	$('.showbtn span').text(0);
	     	$('.qipao').text(0);
	     	$('#buy-DishCaiCount').text(0);
	      	$('#buy-paycount').text(0);
	     	 var checkcost = Number(shoplimitcost);
	          	        $('#shoplimitcost').text('差'+checkcost+'起送');
      	}else{
      		$('.listcontent').remove();
      		$('.colgreen').remove();//移除促销
...
function  orderSubmit(){ 
   ...
   $.ajax({         //script定义
            ...
            dataType: "json",
            async:true,
            data:{shopid:shopid,'remark':$('#remark').val(),'username':$('#username').val(),'mobile':$('#mobile').val(),'addressdet':$('#addressdet').val(),'minit':$("#DeliveryTime").find("option:selected").val(),'senddate':$("#Deliverydate").find("option:selected").val()},
