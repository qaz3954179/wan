...
class wx_b{
	...
	function text(){ 
		...
		if(count($tempinfo) == 2){
		   ...
		   $this->showorder();
			  ...
			  $newlink = $this->Mlink($Mconfig['siteurl'].'/index.php?controller=html5&action=order');
             ...
             if(!empty($shopinfo)){
                ...
                $this->mysql->update($this->tablepre.'shop',$data,"id='".$shopinfo['id']."'");  
             	...
             	$lng =  trim($this->obj->Location_Y);
             	 $shoplist =   $this->mysql->getarr("select id,shopname,lat,lng,shoplogo from ".$this->tablepre."shop  where is_open = 1 and is_pass =1 ORDER BY  (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) ASC limit 0,10");
             	 $contents = '';
             	 if(is_array($shoplist)){
             	 	  ...
             	 	  //  $temc['miaoshu'] = $value['shopname'].'距离您'.$mi;
             	 	   // $temc['biaoti'] = $value['shopname'].'距离您'.$mi;
             	 	    ...
             	 	    $temc['lianjie'] = $Mconfig['siteurl'].'/index.php?controller=html5&action=shop&id='.$value['id'];
             	 	    $tempinfo[] = $temc;
             	 	  }
             	 	  if(count($tempinfo) > 0){
  ...
  function GetDistance($lat1, $lng1, $lat2, $lng2, $len_type = 1, $decimal = 2)
  {
       ...
       if(empty($ehckinfo)){
        ...
        if(!empty($userinfo)){
             ...
             if(!empty($shopinfo)){
                ...
                $this->mysql->update($this->shop.'shop',$data,"uid='".$userinfo['uid']."'");  
             }else{
             	//获取最近 10个店铺
             	//   
	...
	function Mlink($link){
		...
		$linkstr .='openid='.$this->obj->FromUserName.'&actime='.$time.'&sign='.$tempstr;
	  return $linkstr;
	}
	function result(){
	   return $this->result;
